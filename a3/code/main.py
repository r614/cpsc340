...
def q2_1():
    ...
    v_concat = np.concatenate((v_1, v_2), axis=0)
    v = np.diag(v_concat)
...
def q2_4_2():
    ...
    optimizer_2 = OptimizerGradientDescentLineSearch()
    model_2 = linear_models.LinearModelGradientDescent(fun_obj, optimizer_2)
    model_2.fit(X, y)

    # Plot data and model
    plt.figure()
    x_1 = [i+1 for i in range(len(model.fs))]
    plt.plot(x_1, model.fs, color="b", label="opt_gradient_descent" )

    x_2 = [i+1 for i in range(len(model_2.fs))]
    plt.plot(x_2, model_2.fs, color="r", label="opt_gradient_descent_line_search")

    plt.xlabel("Iteration")
    plt.ylabel("F")
    plt.title("Learning curves: Gradient Descent v/s Gradient Descent Line Search")
    plt.legend()
...
def q3_2():
    ...
    for i, (p, ax) in enumerate(zip(p_vals, (ax for row in axes for ax in row))):
        ...
        print(f"validation error = {err_valid} \n\\\\")
