...
class LogisticRegressionLossL2(LogisticRegressionLoss):
    ...
    def evaluate(self, w, X, y):
        ...
        grad = X.T.dot(-y / (1.0 + np.exp(yXw))) + self.lammy * w

        return f, grad


class LogisticRegressionLossL1(LogisticRegressionLoss):
    def __init__(self, lammy = 1) -> None:
        super().__init__()
        self.lammy = lammy 
    
    def evaluate(self, w, X, y):
        """
        Evaluates the function value of of L1-regularized logistics regression objective.
        """
        w = ensure_1d(w)
        y = ensure_1d(y)
...
class SoftmaxLoss(FunObj):
    def evaluate(self, w, X, y):
        ...
        f = -np.sum(xW[y_bin] - np.log(Z))
        g = (np.exp(xW)/Z[:, None] - y_bin).T @ X
