...
def q2_2():
    ...
    for lammy in lammys: 
        ...
        fun_obj = LogisticRegressionLossL1(lammy)
        ...
        print(f"# Features Used: {np.sum(model.w != 0)}")
        print(f"# GD Iterations: {optimizer.num_evals}")
        print("-------\n")
...
def q3_5():
    ...
    all_one, a_name = LogisticRegression(multi_class="ovr", C=10000000000, fit_intercept=False), "sklearn_ovr"
    soft_max, s_name = LogisticRegression(multi_class="multinomial", C=1000000, fit_intercept=False), "sklearn_softmax" 


    for model, name in [(all_one, a_name), (soft_max, s_name)]:
        print(name)
        model.fit(X,y)
