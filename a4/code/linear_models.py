    ...
    def fit(self, X, y):
        ...
        while not np.all(selected):
            ...
            for j in range(d):
                ...
                size = len(selected_with_j[selected_with_j == True])
                w, _, _, _ = self.optimize(np.zeros(size), X[:, selected_with_j], y)
...
class LogRegClassifierOneVsAll(LogRegClassifier):
    ...
    def fit(self, X, y):
        ...
        for i in range(k):
            y_tmp = y.copy().astype(float)
            y_tmp[y==i] = 1
            y_tmp[y!=i] = -1

            W[i], _, _, _  = self.optimize(W[i], X, y_tmp)
